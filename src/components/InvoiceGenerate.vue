<template>
  <div class="invoice-generate">
      ...
      <div class="config-panel">
        ...
        <div class="param-group">
          ...
          <input
            ...
            class="input-field"
          />
          <p class="hint">USD → CNY 参考换算（不填则不转换）</p>
        </div>
        ...
        <div v-if="mappingConfig && Object.keys(mappingConfig).length" class="status-ok">
      ...
      <div class="preview-panel">
        ...
        <div v-if="previewData" class="preview-content">
          ...
          <div class="preview-item">
            ...
            <span>{{ previewData.total_amount || 0 }} {{ previewData.currency || 'USD' }}</span>
          </div>
        </div>
    ...
    <div v-if="downloadLink" class="success-section">
      ...
      <a
        :href="downloadLink"
        :download="`PI_${buyerName || 'invoice'}_${Date.now()}.xlsx`"
        class="download-btn"
      >
        📥 下载 Excel 文件
      </a>
...
export default {
  props: {
    ...
    mappingConfig:  { type: Object, default: null },
  },
      ...
      try {
        ...
        if (contentType.includes('application/json')) {
          ...
          try {
            ...
            this.error = '生成失败：' + (json.message || '服务器返回错误')
          } catch {
            this.error = '生成失败：服务器返回了无效响应'
          }
          return
        }
        ...
        // axios 在非 2xx 时 throw，response.data 也是 blob
        if (err.response && err.response.data instanceof Blob) {
          try {
            const text = await err.response.data.text()
            const json = JSON.parse(text)
            this.error = '生成失败：' + (json.message || '请稍后重试')
          } catch {
            this.error = '生成失败：服务器返回了无效数据'
          }
        } else {
...
.status-ok { padding: 8px 12px; background: #f0fdf4; border: 1px solid #dcfce7; border-radius: 4px; font-size: 13px; color: #059669; margin-bottom: 12px; }
.status-warn { padding: 8px 12px; background: #fffbeb; border: 1px solid #fde68a; border-radius: 4px; font-size: 13px; color: #92400e; margin-bottom: 12px; }
...
.success-section { background: #f0fdf4; border: 1px solid #dcfce7; border-radius: 8px; padding: 24px; text-align: center; margin-top: 16px; }
